...
class SqueakServer {
    ...
    serve(req, res) {
        ...
        if (this.fileCache && this.squeakCache.check(req.url))
            return this.serveCache(req, res);
    ...
    // Serve Methods
    serveView(req, res) {
        if (this.viewStore[req.url].onRequest)
            this.viewStore[req.url].onRequest(req);
        res.writeHead(200, { 'Content-Type': 'text/html' });
        return res.end(this.viewStore[req.url].viewRender, 'utf-8');
    }
    serveInterface(req, res) {
        if (this.interfaceStore[req.url].onRequest)
            this.interfaceStore[req.url].onRequest(req);
        this.interfaceStore[req.url].__request(req, res);
        if (this.interfaceStore[req.url].afterRequest)
            this.interfaceStore[req.url].afterRequest(req);
        return;
    }
    ...
    serveCache(req, res) {
        if (this.viewStore[req.url] && this.viewStore[req.url].onRequest)
            this.viewStore[req.url].onRequest(req);
        ...
        if (this.viewStore[req.url] && this.viewStore[req.url].afterRequest)
            this.viewStore[req.url].afterRequest(req);
        return;
    }
    tryFile(req, res) {
        ...
        const contentType = squeakconst_1.MIMETYPES[extname] || 'application/octet-stream';
